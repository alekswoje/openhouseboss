...
actor APIClient {
    ...
    func createSession(audioURL: URL) async throws -> Session {
        try await createSession(audioURL: audioURL, visitorsCSV: nil)
    }

    // Kiosk path: upload audio plus a sign-in CSV so the backend can match
    // named visitors to speakers.
    func createSession(audioURL: URL, visitors: [VisitorInput]) async throws -> Session {
        let csv = (["name,email,phone,signed_in_at"]
            + visitors.map { "\($0.name),\($0.email),\($0.phone)," }).joined(separator: "\n")
        return try await createSession(audioURL: audioURL, visitorsCSV: Data(csv.utf8))
    }

    private func createSession(audioURL: URL, visitorsCSV: Data?) async throws -> Session {
